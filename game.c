...
void spawnpoison(SDL_FRect *poison, SDL_FRect *body, int snakeLength, SDL_FRect *food)
{
        ...
        if (!gameOver && !isPaused)
        {
            ...
            {     countPoint+=1;
                ...
                if (countPoint % 3 == 0 && countPoint != 0) {
                    spawnpoison(&poison, snakeBody, snakeLength, &food);
                    isPoisonActive = true;
                }
                spawnFood(&food, snakeBody, snakeLength);
            ...
            if (isPoisonActive && collisionWithpoison(&snakeBody[0], &poison))
            {
                score -= 10;
                if (snakeLength > 1)
                {
