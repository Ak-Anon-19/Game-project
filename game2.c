...
void spawnpoison(SDL_FRect *poison, SDL_FRect *body, int snakeLength)
{
    bool ispoisonOnSnake = true;
    while (ispoisonOnSnake)
    {
        int randomCol = rand() % GRID_COLS;
        int randomRow = rand() % GRID_ROWS;

        poison->x = (float)(randomCol * GRID_SIZE);
        poison->y = (float)(randomRow * GRID_SIZE);
        poison->w = GRID_SIZE;
        poison->h = GRID_SIZE;

        ispoisonOnSnake = false;

        for (int i = 0; i < snakeLength; i++)
        {
            if (poison->x == body[i].x && poison->y == body[i].y)
            {
                ispoisonOnSnake = true;
                break;
            }
        }
    }
...
bool collisionWithpoison(SDL_FRect *head, SDL_FRect *poison)
{
...
int main(int argc, char *argv[])
{
    ...
    spawnpoison(&poison, snakeBody, snakeLength);
    ...
    while (isRunning)
    {
        ...
        if (!gameOver && !isPaused)
        {
            ...
            {     countPoint+=1;
             ...
             if (collisionWithpoison(&snakeBody[0], &poison) &&countPoint%3==0 && countPoint!=0 && psn ==true)
            {    // countPoint+=1;
                score -= 10;
                if (snakeLength < MAX_SNAKE_LENGTH)
                {
        ...
        if (!gameOver)
        {
            ...
            if(countPoint%3==0&& countPoint!=0){
            SDL_SetRenderDrawColor(renderer, 0, 255, 0, 255);
            SDL_RenderFillRect(renderer, &poison);
            }
